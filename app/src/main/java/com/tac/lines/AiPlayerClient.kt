...
object AiPlayerClient {

    private const val AI_PLAY_STEP_URL = "https://taclines-backend.onrender.com/ai-play-step"
    ...
    fun playStep(
        ...
        return try {
            ...
            val conn = URL(AI_PLAY_STEP_URL).openConnection() as HttpURLConnection
            ...
            try {
                ...
                val parsed = parseAiPlayStep(body)
    ...
    private fun parseAiPlayStep(json: String): AiPlayStep {
        ...
        val shouldShoot = root.optBoolean("shouldShoot", false)
    ...
    private fun parseGesture(obj: JSONObject?): AiPlayGesture? {
        ...
        val durationMs = obj.optLong("durationMs", 220L)

        if (fromX < 0f || fromY < 0f || toX < 0f || toY < 0f) return null

        return AiPlayGesture(
            fromX = fromX,
            fromY = fromY,
            toX = toX,
            toY = toY,
    ...
    private fun scaleStepBack(
        step: AiPlayStep,
        scaleX: Float,
        scaleY: Float
    ): AiPlayStep {
        val g = step.gesture ?: return step

        val scaledGesture = AiPlayGesture(
